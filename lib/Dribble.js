...
Dribble.prototype.addFormat = function (key, formatter) {

    formatters[key] = formatter;

}

Dribble.prototype.addLevel = function (level) {

    Dribble.prototype[level] = function (message, obj, location, cb) {
        this.log(level, message, obj, location, cb);
    }

    this.on(level, function (message, obj, location, cb) {
        this.process(level, message, obj, location, cb);
    });
...
Dribble.prototype.getGroupLabels = function () {
    ...
    Object.keys(level_config).forEach(function (k) {
        ...
        if (level_config[k].disabled !== true) {
            ...
            if (level_config[k].include) {
                if (level_config[k].include.indexOf('stack') !== -1 && (meta instanceof Error || meta instanceof ReferenceError)) {
            ...
            switch (k) {
                ...
                case 'console':
                    ...
                    var formatted = formatters[level_config[k].fileFormat || 'plain'](cmessage, meta, level_config[k]);

                    if (level_config[k].color) {
                        var style = colors[level_config[k].color],
                            formatted = style[0] + formatted + style[1];
                ...
                case 'file':
                    ...
                    if (path.indexOf('{source}') !== -1) {
                        path = path.replace('{source}', location || 'process');
                    }
                    ...
                    var formatted_data = formatters[level_config[k].format || level_config[k].fileFormat || 'json'](message, meta, level_config[k]);
                    writeToFile(path, formatted_data + ',\n');
                    break;

                case 'socket':
                    me.writeToSocket.call(me, {msg:message, meta:meta});
...
Dribble.prototype.matchTransports = function (level) {
    ...
    if (!return_cfg) {
        ...
        transports.forEach(function (t) {
            ...
            //
            _.defaults(return_cfg[t], _.pick(level_config, 'include', 'disabled', 'exclude', 'format'));
...
Dribble.prototype.writeToSocket = function (data) {
    if (this.socket) {
